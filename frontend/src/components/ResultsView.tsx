    ...
    <div className="w-full bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 uppercase">
              {result.action}
            </span>
          </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          {result.artifact?.download_url && (
            <a
              ...
              className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs transition-all shadow-md shadow-indigo-600/20"
            >
      ...
      <div className="flex items-center space-x-2 bg-slate-950/80 p-1 rounded-xl border border-slate-800 text-xs w-fit">
        ...
        >
          ...
          <span>{isHtmlViz ? "Interactive Chart" : "Result Artifact"}</span>
        </button>
          ...
          {isHtmlViz && (result.artifact?.html_content || result.artifact?.download_url) ? (
            <div
              className={`relative rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden ${
                isFullscreen ? "fixed inset-4 z-50 p-4 shadow-2xl bg-slate-950" : "h-[560px]"
              }`}
            >
              {/* Plotly Frame Controls */}
              <div className="absolute top-3 right-3 z-10 flex items-center space-x-2 bg-slate-900/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-slate-800 text-xs">
                <button
                  type="button"
            ...
            <div className="p-8 rounded-2xl bg-slate-950/80 border border-slate-800 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div className="max-w-md">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Your dataset was transformed and exported inside the sandbox environment.
                </p>
              </div>

              {result.stdout && (
                <div className="w-full max-w-lg p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-left font-mono text-[11px] text-slate-300">
      ...
      {activeTab === "logs" && (
        <div className="rounded-xl border border-slate-800 bg-[#090d16] p-4 font-mono text-xs text-slate-300 space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            ...
            <span className="text-[10px] text-slate-500">Exit Code: {result.success ? "0 (SUCCESS)" : "1 (ERROR)"}</span>
          </div>
